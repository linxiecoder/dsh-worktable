...
let hostApi: { agentPresets?: any; sessions?: any; host?: any } | null = null
...
function WorktableSection(props: any) {
  ...
  // 新建项目强制工作文件夹：优先调宿主选择器；不可用时允许直接填写绝对路径。
  ...
  const folderBrowserApplyRef = useRef<((path: string) => void) | null>(null)
  const folderBrowserRequestRef = useRef(0)
  ...
  const defaultLayoutName = () => {
    const index = String(projects.layouts.length + 1)
    return t('add.defaultName', { index }).replace('{index}', index)
  }
  ...
  const openAddPanel = () => {
    ...
    setFolderPickError('')
    setWsName((current) => current.trim() ? current : defaultLayoutName())
    if (!wsFolderParent) {
...
function buildCustomLayoutPrompt(req: string): string {
  ...
  /** 内置目录浏览器：原生选择器不可用时仍能在 Desktop 内完成文件夹选择。 */
  const loadFolderBrowser = async (requestedPath?: string) => {
    const path = requestedPath?.trim() ?? ''
    const requestId = ++folderBrowserRequestRef.current
    setFolderBrowser((previous) => ({
    ...
    try {
      ...
      if (!response.ok || typeof payload?.path !== 'string' || !payload.path) throw new Error('directory listing unavailable')
      if (requestId !== folderBrowserRequestRef.current) return
      const entries = Array.isArray(payload.entries)
        ? payload.entries.filter((entry: any): entry is DirectoryEntry => (
          entry && typeof entry.name === 'string' && typeof entry.path === 'string' && entry.isDir === true
        ))
        : []
      setFolderBrowser({
        path: payload.path,
      ...
      setFolderBrowser((previous) => ({
        ...
        truncated: previous?.truncated ?? false,
        loading: false,
        error: t('add.folderBrowserFail'),
      }))
    }
  }

  const closeFolderBrowser = () => {
    folderBrowserRequestRef.current += 1
    folderBrowserApplyRef.current = null
    setFolderBrowser(null)
  }

  const openFolderBrowser = (apply: (path: string) => void) => {
    folderBrowserApplyRef.current = (path) => {
      apply(path)
      setFolderPickError('')
    }
    const initialPath = wsFolderParent.trim() || sessionScopeStore.snapshot?.cwd?.trim() || ''
    void loadFolderBrowser(initialPath)
  }

  const chooseFolderBrowserPath = () => {
    const path = folderBrowser?.path.trim()
    if (!path) return
    folderBrowserApplyRef.current?.(path)
    closeFolderBrowser()
  }
  ...
  /**
   * 优先调用宿主原生文件夹选择窗。若宿主桥接不可用，则降级到本插件的目录浏览器，
   * 不再把用户退回到只能手工输入路径的死路。
   */
  const pickFolder = async (apply: (p: string) => void) => {
    if (folderPickBusy) return
    setFolderPickBusy(true)
    setFolderPickError('')
    try {
      const tries: Array<() => Promise<any>> = []
      const workspaces = sessionBridge?.workspaces as any
      if (workspaces && typeof workspaces.pickDirectory === 'function') {
        tries.push(() => workspaces.pickDirectory())
      }
      const host = hostApi?.host as any
      if (host && typeof host.pickDirectory === 'function') {
        // 保留 host 作为 this；直接解构方法会让 DSH 的连接 API 丢失调用上下文。
        tries.push(() => host.pickDirectory({}))
      }
          ...
          const path = typeof value === 'string'
            ...
            : typeof value?.path === 'string'
              ...
              : typeof value?.result?.value === 'string'
                ? value.result.value
                : typeof value?.result?.value?.path === 'string'
                  ? value.result.value.path
                  : typeof value?.result?.value?.folder === 'string'
                    ? value.result.value.folder
                    : ''
          if (path) { apply(path); return }
        } catch {
          // 尝试下一个宿主入口；全部失败时由内置浏览器接管。
        }
      }
      openFolderBrowser(apply)
    } finally {
  ...
  const saveLayout = async () => {
    const name = wsName.trim() || defaultLayoutName() || `工作台 ${projects.layouts.length + 1}`
  ...
  // + 弹窗：一般面板向右弹出；设置面板留在侧边栏内部，不能遮挡聊天区。
  ...
  // 设置面板处于侧栏文档流中，不遮挡聊天区；侧栏外点击可收起。
  useEffect(() => {
    if (!viewOptionsOpen) return
    const onPointerDown = (event: PointerEvent) => {
      const target = event.target as Node | null
      if (!target || settingsRef.current?.contains(target) || settingsTriggerRef.current?.contains(target)) return
      setViewOptionsOpen(false)
    }
  ...
  return (
    <div ref={rootRef} className={'dsh-wt_section' + (isFloat ? ' dsh-wt_float' : '')} style={isFloat ? floatStyle : dockedStyle}>
        ...
        <div className="dsh-wt_menu dsh-wt_add dsh-wt_pop" style={{ position: 'fixed', left: popLeft, top: popTop, width: 360, zIndex: 80 }}>
          ...
          <div className="dsh-wt_addForm">
            ...
            <div className="dsh-wt_addFolderRow">
              ...
              <input
                type="text"
                className="dsh-wt_addFolderPath"
                aria-label={t('add.folderParent')}
                placeholder={t('add.folderNone')}
                value={wsFolderParent}
                onChange={(e) => { setWsFolderParent(e.target.value); setWsFolderError(false); setFolderPickError('') }}
              />
              <button
                type="button"
                className="dsh-wt_bindFolderChange"
                disabled={folderPickBusy}
                aria-busy={folderPickBusy || undefined}
                onClick={() => { void pickFolder((p) => { setWsFolderParent(p); setWsFolderError(false) }); setWsError(false) }}
              >{folderPickBusy ? t('add.folderPicking') : t('add.folderPick')}</button>
            </div>
          ...
          {folderPickError && <p className="dsh-wt_addError" role="alert">{folderPickError}</p>}
        </div>
      )}

      {folderBrowser && (
        <>
          <div className="dsh-wt_popBackdrop" style={{ zIndex: 88 }} onClick={closeFolderBrowser} />
          <div className="dsh-wt_folderBrowser" role="dialog" aria-modal="true" aria-label={t('add.folderBrowserTitle')}>
            <div className="dsh-wt_folderBrowserHead">
              <span>{t('add.folderBrowserTitle')}</span>
              <button type="button" className="dsh-wt_settingsClose" aria-label={t('add.folderBrowserCancel')} onClick={closeFolderBrowser}>×</button>
            </div>
            <div className="dsh-wt_folderBrowserPathRow">
              <input
                type="text"
                aria-label={t('add.folderBrowserPath')}
                value={folderBrowser.path}
                onChange={(event) => setFolderBrowser((current) => current ? { ...current, path: event.target.value, error: '' } : current)}
                onKeyDown={(event) => { if (event.key === 'Enter') void loadFolderBrowser(folderBrowser.path) }}
              />
              <button type="button" disabled={folderBrowser.loading} onClick={() => void loadFolderBrowser(folderBrowser.path)}>{t('add.folderBrowserRefresh')}</button>
            </div>
            <div className="dsh-wt_folderBrowserNav">
              <button
                type="button"
                disabled={folderBrowser.loading || !folderBrowser.path || parentPathOf(folderBrowser.path) === folderBrowser.path}
                onClick={() => void loadFolderBrowser(parentPathOf(folderBrowser.path))}
              >← {t('add.folderBrowserUp')}</button>
              <span title={folderBrowser.path}>{folderBrowser.path || '…'}</span>
            </div>
            <div className="dsh-wt_folderBrowserList" aria-busy={folderBrowser.loading || undefined}>
              {folderBrowser.loading
                ? <p>{t('add.folderPicking')}</p>
                : folderBrowser.error
                  ? <p className="dsh-wt_addError" role="alert">{folderBrowser.error}</p>
                  : folderBrowser.entries.length === 0
                    ? <p>{t('add.folderBrowserEmpty')}</p>
                    : folderBrowser.entries.map((entry) => (
                      <button key={entry.path} type="button" title={entry.path} onClick={() => void loadFolderBrowser(entry.path)}>
                        <span aria-hidden>📁</span>
                        <span>{entry.name}</span>
                      </button>
                    ))}
            </div>
            {folderBrowser.truncated && <p className="dsh-wt_folderBrowserHint">{t('add.folderBrowserTruncated')}</p>}
            <div className="dsh-wt_folderBrowserActions">
              <button type="button" onClick={closeFolderBrowser}>{t('add.folderBrowserCancel')}</button>
              <button type="button" className="dsh-wt_addBtn" disabled={!folderBrowser.path || folderBrowser.loading} onClick={chooseFolderBrowserPath}>{t('add.folderBrowserSelect')}</button>
            </div>
          </div>
        </>
      )}
      ...
      {viewOptionsOpen && (
        <div ref={settingsRef} className="dsh-wt_manage dsh-wt_settings dsh-wt_settingsInline">
          <button
            type="button"
        ...
        <div className="dsh-wt_menu dsh-wt_pop dsh-wt_bindPop" style={{ position: 'fixed', left: bindPick.x, top: bindPick.y, width: 280, zIndex: 84 }}>
          ...
          <div className="dsh-wt_bindFolderBox">
            <div className="dsh-wt_bindFolderRow">
              ...
              <button type="button" className="dsh-wt_bindFolderChange" disabled={folderPickBusy} aria-busy={folderPickBusy || undefined} onClick={() => void changeBindFolder()}>{folderPickBusy ? t('add.folderPicking') : t('bind.folderChange')} ↻</button>
            </div>
            ...
            {folderPickError && <p className="dsh-wt_bindFolderError" role="alert">{folderPickError}</p>}
          </div>
          ...
          <div className="dsh-wt_bindFolderBox">
            ...
            {(() => {
              ...
              return (
                <button type="button" className="dsh-wt_bindConvRow" aria-label={t('bind.tipBound', { name: info.title })} onClick={() => setBindListOpen((v) => !v)}>
